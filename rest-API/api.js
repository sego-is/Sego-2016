(function () {
  ...
  api.get('/companies/customers/:company_id', (req, res) => {
    model.Person.find({
      ...
      'role': 0
    }).populate('history').exec(function (err, persons) {
      if (err) {
        res.status(500).send(err);
      } else {
        res.send(persons);
...
// SKODA ADEINS URI HER, VAR AD CONFLICTA WITH api.get('/bookings/:cid/:date') svo breytti i book
  ...
  api.get('/bookings/:cid/:date', (req, res) => {
    ...
    }).populate('bookings.customer_id bookings.staff_id').exec(function (err, docs) {
      if (err) {
        res.status(500).send(err);
      } else {
        if (docs.length === 0) {
  ...
  api.post('/persons', bodyParser.json(), (req, res) => {
    ...
    if (data._id === undefined) {
      ...
      model.Person.update({'_id': {$eq: data._id}}, {
        '$set': {
          'name': data.name,
          'email': data.email,
  ...
  api.post('/bookings/:bid/notAttend', bodyParser.json(), (req, res) => {
    ...
    model.Booking.update({
      ...
      'date': {$eq: data.date},
      'bookings._id': {$eq: req.params.bid}
    }, {
      '$set': {
